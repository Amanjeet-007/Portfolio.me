...
export default function Connect() {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => setStatus({ type: null, text: '' }), 5000);
  ...
  const sendEmail = (e) => {
    ...
    emailjs
      ...
      .then(
        ...
        (error) => {
          setStatus({ type: 'error', text: `Couldn't send it \u2014 ${error.text}` });
        }
      )
  ...
  const social = [
    {
      ...
      img: (
        <svg width="32" height="32" xmlns="http://www.w3.org/2000/svg" viewBox="0 -0.5 41 41">
          <g fill="none" strokeMiterlimit="10">
            <path fill="#ffe236" stroke="#231f20" d="M36 33.59c3-1.36 3.53-8.34 3.53-13.59S38.94 7.77 36 6.41c-1.48-1.36-9.4-1.73-16-1.73S5.52 5.05 4 6.41C1.06 7.77.5 14.75.5 20S1.06 32.23 4 33.59c1.49 1.36 9.41 1.73 16 1.73s14.49-.32 16-1.73Z" />
            <path stroke="#231f20" strokeLinecap="round" d="M4.67 13c3.65 3.81 8.94 7.93 10.57 8.64a10.81 10.81 0 0 0 9.52 0c1.62-.64 6.91-4.79 10.56-8.64m-20.89 8.39l-6.96 6.97m18.1-6.97l6.97 6.97" />
            <path stroke="#fff" strokeLinecap="round" d="M29.05 7.53a15.4 15.4 0 0 1 5.27.92" />
          </g>
        </svg>
      ),
      accent: '#ffe236',
  ...
  const fieldClass =
    'w-full bg-white/5 border border-white/15 rounded-xl px-4 py-3 text-base text-white placeholder:text-white/35 outline-none transition-all duration-200 focus:border-[#DA0037] focus:bg-white/[0.08] focus:ring-2 focus:ring-[#DA0037]/25';

  const labelClass = 'block text-sm font-semibold uppercase tracking-wide text-white/50 mb-2';

  return (
    <main className="relative w-full min-h-screen text-white overflow-hidden">
      {/* ambient background glows, matches the site's dark theme */}
      <div className="pointer-events-none absolute -top-40 -left-40 w-[500px] h-[500px] rounded-full  blur-[120px]" />
      <div className="pointer-events-none absolute -bottom-40 -right-40 w-[500px] h-[500px] rounded-full bg-[#06D001]/10 blur-[120px]" />
      ...
      <div className="relative z-10 w-full max-w-6xl mx-auto px-6 md:px-10 py-20">
        ...
        >
          <span className="inline-block text-sm font-bold tracking-[0.2em] uppercase text-[#DA0037] mb-3">
            Get In Touch
          </span>
          <h2 className="text-4xl md:text-5xl font-extrabold">
            Let&rsquo;s Build{' '}
            <span className="text-[#06D001]">Something</span> Together
          </h2>
          <p className="text-white/50 mt-4 max-w-xl mx-auto">
            Have a project in mind, or just want to say hi? Drop a message below
            or reach out directly on any platform.
          </p>
        </motion.div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1.2fr_1fr] gap-8 items-start">
          ...
          <motion.div
            ...
            viewport={{ once: true }}
            transition={{ duration: 0.6 }}
            className="bg-white/[0.04] backdrop-blur-xl border border-white/10 rounded-3xl p-6 md:p-10 shadow-2xl"
          >
            <form ref={form} onSubmit={sendEmail} className="space-y-6">
              ...
              <motion.button
                ...
                className={`w-full flex items-center justify-center gap-2 py-3.5 rounded-xl font-bold text-white transition-opacity ${
                  isSending
                    ? 'bg-[#DA0037]/50 cursor-not-allowed'
                    : 'bg-linear-to-r from-[#DA0037] to-[#ff4d6d] cursor-pointer'
                }`}
              >
                {isSending ? (
                  <>
                    <motion.span
                      ...
                      className="h-4 w-4 border-2 border-white/40 border-t-white rounded-full inline-block"
                    />
                    Sending...
                  </>
                ) : (
                ...
                {status.type && (
                  <motion.div
                    ...
                    className={`flex items-center gap-2 px-4 py-3 rounded-xl text-sm font-medium ${
                      status.type === 'success'
                        ? 'bg-[#06D001]/10 text-[#06D001] border border-[#06D001]/30'
                        : 'bg-[#DA0037]/10 text-[#ff6b81] border border-[#DA0037]/30'
                    }`}
                  >
          ...
          <motion.div
            ...
            viewport={{ once: true }}
            transition={{ duration: 0.6 }}
            className="bg-white/[0.04] backdrop-blur-xl border border-white/10 rounded-3xl p-6 md:p-10 shadow-2xl h-full"
          >
            <img src="/wallchar.webp" alt="character" className='absolute h-full left-[80%] top-0 md:flex hidden ' />
            <h3 className="text-2xl font-bold mb-1">Reach Me Directly</h3>
            <p className="text-white/45 text-sm mb-8">
              Pick whatever platform works best for you.
            </p>
            ...
            >
              {social.map((el) => (
                <motion.a
                  ...
                  className="flex flex-col items-center justify-center gap-3 rounded-2xl py-6 px-4 bg-white/[0.03] border border-white/10 transition-colors"
                  style={{ '--accent': el.accent }}
                  onMouseEnter={(e) => (e.currentTarget.style.borderColor = el.accent)}
                  onMouseLeave={(e) => (e.currentTarget.style.borderColor = 'rgba(255,255,255,0.1)')}
                >
                  {el.img}
                  <span className="text-sm font-semibold text-white/80">{el.name}</span>
                </motion.a>
              ))}
            </motion.div>
          </motion.div>
