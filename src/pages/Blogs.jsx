      ...
      <div className="blogs w-full h-full flex items-center justify-around flex-wrap ">
        {
          blogs.map(el => {
            return (
              <motion.div
                className="card h-80 w-90 m-10 rounded-2xl flex items-center flex-col overflow-hidden text-center text-white cursor-pointer border-2 border-black"
                whileHover={{ scale: 1.1 }}
              >
              <Link to={el.path}>
                <p className="">{el.date}</p>
                <motion.img src={el.image} alt="" className="h-40 bg-white w-[100%] rounded-2xl border-1 border-grey-500 shadow-2xl shadow-gray-700" whileHover={{scale:.9}} />
